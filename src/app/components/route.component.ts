...
@Component({
  ...
  template: `
    <div class="route"
      ...
      <div class="journey-icon-container _site"
           *ngIf="$data.currentGuiContext_notPersitent.currentJourneySiteTo as card">
        <div class="spacing"></div>
        <div class="journey-icon _site"
             (click)="$app.haveModalReversed = reversed; $app.zoomCard = $data.currentGuiContext_notPersitent.currentJourneySiteTo"
      ...
      <div class="journey-icon-container" *ngIf="$data.currentGuiContext_notPersitent.currentPlayableHazards.length">
        ...
        <div class="journey-icon _hazard"
             (click)="$app.haveModalReversed = reversed; $app.openHazardCardsModal = true">
        </div>
      </div>
    </div>
  `,
